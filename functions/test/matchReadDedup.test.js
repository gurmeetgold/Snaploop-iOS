...
test("missing capture timestamp is preserved rather than treated as zero", () => {
  const first = photo({ id: "first", capturedAtMillis: null });
  const second = photo({ id: "second", capturedAtMillis: null });
  assert.deepEqual(deduplicateMatchedPhotos([first, second]), [first, second]);
});
