...
exports.acceptBiometricConsent = onCall(async (request) => {
  ...
  if (requestedVersion !== CONSENT_POLICY_VERSION) throw new HttpsError("failed-precondition", "Please review the current Face Match Consent before continuing.");
  ...
  return { accepted: true, policyVersion: CONSENT_POLICY_VERSION, acceptedAtMillis: acceptedAt.toMillis() };
});
...
exports.listEventFaceProfiles = onCall(async (request) => {
  ...
  if (event.status !== "active") throw new HttpsError("failed-precondition", "Face matching is available only for an active Trip.");
...
// Ended Trips are fully removed no later than 15 days after their selected end.
...
// Manually deleted Trips follow the same 15-day maximum, anchored to deletion.
