...
struct EventDashboardView: View {
    ...
    init(event: Event) { _currentEvent = State(initialValue: event) }
    ...
    private func syncRosterIdentities() async throws {
        let _: Any = try await withCheckedThrowingContinuation { continuation in
            Functions.functions().httpsCallable("syncEventRosterIdentities").call(["eventId": currentEvent.id]) { result, error in
    ...
    private var hero: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            Image(systemName: currentEvent.category.systemImage).font(.system(size: 92, weight: .semibold)).foregroundStyle(.white.opacity(0.13)).offset(x: 230, y: -30)
            VStack(alignment: .leading, spacing: 10) {
                HStack(alignment: .top) {
                    ZStack { RoundedRectangle(cornerRadius: 16).fill(.white.opacity(0.18)); Image(systemName: currentEvent.category.systemImage).font(.title2).foregroundStyle(.white) }.frame(width: 52, height: 52)
                    Spacer(); lifecyclePill
                }
                Spacer()
                Text(currentEvent.name).font(.system(size: 30, weight: .bold, design: .rounded)).foregroundStyle(.white).lineLimit(2)
                HStack(spacing: 8) {
                    Label(DateFormatting.range(currentEvent.startsAt, currentEvent.endsAt), systemImage: "calendar").font(.caption.weight(.semibold)).foregroundStyle(.white.opacity(0.92))
                    if let role = currentUserRole { Label(role.displayName.uppercased(), systemImage: role == .organizer ? "crown.fill" : role == .admin ? "shield.fill" : "person.fill").font(.caption2.bold()).padding(.horizontal, 9).padding(.vertical, 5).background(.white.opacity(0.18), in: Capsule()).foregroundStyle(.white) }
                }
            }.padding(20)
        }.frame(height: 230).clipShape(RoundedRectangle(cornerRadius: 30)).shadow(color: Theme.sunset.opacity(0.18), radius: 18, y: 10).padding(.horizontal)
    }

    private var lifecyclePill: some View { Text(statusText).font(.caption2.bold()).padding(.horizontal, 10).padding(.vertical, 6).background(.white.opacity(0.22), in: Capsule()).foregroundStyle(.white) }
    private var statusText: String {
        switch currentEvent.status {
        case .endedByOrganizer: return "ENDED"
        case .deletedByOrganizer: return "DELETED"
        case .expired: return "COMPLETED"
        case .active: switch lifecycle { case .upcoming: return "UPCOMING"; case .active: return "LIVE"; case .grace: return "WRAPPING UP"; case .expired: return "COMPLETED" }
        }
    }
    ...
    private var featureGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
            NavigationLink { MyPhotosView(event: currentEvent) } label: { GradientTile(title: "My Photos", subtitle: "\(photosOfMe) found of you", systemImage: "person.crop.rectangle.stack.fill", gradient: Theme.brandGradient) }
            NavigationLink { SyncView(event: currentEvent) } label: { GradientTile(title: "Sync Camera", subtitle: "Find new photos", systemImage: "arrow.triangle.2.circlepath", gradient: Theme.socialGradient) }
    ...
    private func memberAvatar(_ member: EventMember) -> some View {
        ZStack { Circle().fill(member.role == .organizer ? Theme.brandGradient : member.role == .admin ? Theme.violetGradient : Theme.socialGradient); Text(initial(for: member)).font(.caption.bold()).foregroundStyle(.white) }
            .frame(width: 42, height: 42).overlay(Circle().strokeBorder(.white, lineWidth: 2))
    }
    ...
    private var invitePeopleRow: some View {
        NavigationLink { ShareEventView(event: currentEvent) } label: {
            PremiumCard {
                HStack(spacing: 12) {
                    ZStack { RoundedRectangle(cornerRadius: 12).fill(Theme.sunset.opacity(0.12)); Image(systemName: "person.badge.plus").font(.title3).foregroundStyle(Theme.sunset) }.frame(width: 42, height: 42)
                    VStack(alignment: .leading, spacing: 2) { Text("Invite People").font(.headline).foregroundStyle(Theme.ink); Text("Share code, link, QR or phone invite").font(.caption).foregroundStyle(.secondary) }
    ...
    private func initial(for member: EventMember) -> String {
        if member.userId == session.user?.id { if let name = session.user?.displayName, let first = name.first { return String(first).uppercased() }; if let phone = session.user?.phoneNumber { return String(phone.suffix(2)) } }
        if let participant = participants.first(where: { $0.userId == member.userId }) { if let name = participant.displayName, let first = name.first { return String(first).uppercased() } }
        return "?"
    }
    ...
    private var managerControls: some View {
        PremiumCard {
            VStack(alignment: .leading, spacing: 12) {
                Label(currentUserRole == .admin ? "Admin Controls" : "Organizer Controls", systemImage: currentUserRole == .admin ? "shield.fill" : "crown.fill").font(.subheadline.bold()).foregroundStyle(Theme.ink)
    ...
    private func changeStatus(_ operation: @escaping () async throws -> Void) async {
        isChangingStatus = true; actionError = nil; defer { isChangingStatus = false }
        do { try await operation(); session.activeEvent = nil; dismiss() }
